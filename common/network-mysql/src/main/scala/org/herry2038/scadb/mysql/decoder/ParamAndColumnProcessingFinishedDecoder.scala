...
import org.herry2038.scadb.mysql.message.server.{ParamAndColumnProcessingFinishedMessage, ServerMessage}
...
object ParamAndColumnProcessingFinishedDecoder extends MessageDecoder {
  ...
  def encode(message: ServerMessage): ByteBuf = {
    val pcpf = message.asInstanceOf[ParamAndColumnProcessingFinishedMessage]
    
    EOFMessageDecoder.encode(pcpf.eofMessage)
