...
import org.herry2038.scadb.mysql.message.server.{ParamProcessingFinishedMessage, ServerMessage}
import org.herry2038.scadb.mysql.message.server.{ParamProcessingFinishedMessage, ServerMessage}
...
object ParamProcessingFinishedDecoder extends MessageDecoder {
  ...
  def encode(message: ServerMessage): ByteBuf = {
    val ppf = message.asInstanceOf[ParamProcessingFinishedMessage]
    
    EOFMessageDecoder.encode(ppf.eofMessage)
