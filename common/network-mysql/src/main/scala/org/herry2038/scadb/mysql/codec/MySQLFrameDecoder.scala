...
import org.herry2038.scadb.mysql.decoder._
...
import org.herry2038.scadb.mysql.decoder.{ParamProcessingFinishedDecoder, ParamAndColumnProcessingFinishedDecoder}
import org.herry2038.scadb.mysql.message.server.ServerMessage
import org.herry2038.scadb.util.Log


class MySQLFrameDecoder(charset: Charset, connectionId: String, returnStr: Boolean = false) extends ByteToMessageDecoder {
  ...
  private final val columnDecoder = new ColumnDefinitionDecoder(charset, new DecoderRegistry(charset), returnStr)
  ...
  private final val loadlocalDecoder = new LoadLocalMsgDecoder(charset)
  ...
  private def handleCommonFlow(messageType: Byte, slice: ByteBuf, out: java.util.List[Object]) {
    val decoder = messageType match {
      ...
      case ServerMessage.LoadLocal => {
        if ( hasReadColumnsCount ) // 20170313  row数据里面也有可能第一个字段为null,就会跟loadlocal 混淆
          null
        else
          this.loadlocalDecoder
      }
      case _ => {
        if (this.isInQuery) {
          null
