...
import com.alibaba.druid.sql.ast.statement.{SQLDeleteStatement, SQLExprTableSource}
 ...
 */
class DecoratorInterfaceDelete(statement: SQLStatement, startTime: Long) extends DecoratorInterface(statement, startTime) {
  val readable = false
  val stmt = statement.asInstanceOf[SQLDeleteStatement]
  val tableName = stmt.getTableName.getSimpleName()
  override val requestType = Statistics.DELETE_TYPE
